...
// ==========================================
// --- CORE ATOMIC COMPONENT MODULES ---
// ==========================================

function CapacityAnalyzer({ assignedVideos, totalHours }) {
  const capPercentage = Math.min(100, Math.round((totalHours / 40) * 100));
  let statusColor = "bg-[#B1E55A]";
  let labelText = "OPTIMAL WORKLOAD SLOTS";
  if (capPercentage > 85) {
    statusColor = "bg-[#FF6B6B]";
    labelText = "CRITICAL CAPACITY EXCEEDED";
  } else if (capPercentage > 60) {
    statusColor = "bg-[#FFD93D]";
    labelText = "HEAVY RUN OPERATIONS";
  }

  return (
    <div className="w-full bg-[#F3E8FF] border-4 border-black p-6 shadow-[5px_5px_0px_0px_#000000] font-mono rounded-2xl">
      <div className="flex items-center gap-2 border-b-4 border-black pb-3 mb-4">
        <span className="text-xl">📊</span>
        <h2 className="text-xs font-black tracking-wider uppercase">Capacity Loading Matrix</h2>
      </div>
      <div className="space-y-4">
        <div className="flex justify-between text-xs font-black uppercase">
          <span>Active Pipeline Assets:</span>
          <span>{assignedVideos} Units Active</span>
        </div>
        <div className="flex justify-between text-xs font-black uppercase">
          <span>Weekly Time Allocations:</span>
          <span>{totalHours} Hrs / 40 Max</span>
        </div>
        <div className="border-4 border-black bg-white p-2 rounded-xl">
          <div className="w-full bg-gray-200 h-6 border-2 border-black relative rounded overflow-hidden">
            <div className={`h-full border-r-2 border-black transition-all duration-500 ${statusColor}`} style={{ width: `${capPercentage}%` }} />
            <span className="absolute inset-0 flex items-center justify-center text-[10px] font-black text-black">
              {capPercentage}% UTILITY RATE
            </span>
          </div>
        </div>
        <div className={`border-2 border-black px-3 py-1.5 rounded-lg text-center font-black text-[10px] uppercase tracking-wide ${statusColor}`}>
          👉 {labelText}
        </div>
      </div>
    </div>
...
// ==========================================
// --- NEW FEATURE 3: PREDICTION ENGINE ---
// ==========================================
...
export default function App() {
  ...
  const totalWeeklyQuotaHours = useMemo(() => {
    return clients.reduce((accum, cur) => accum + (cur.videosPerWeek * 3.5), 0);
  }, [clients]);
